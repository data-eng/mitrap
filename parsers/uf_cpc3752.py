...
if instrument_tz == "UTC":
    df["datetime"] = pandas.to_datetime( df["Date-Time"], format='%Y-%m-%d %H:%M:%S', utc=True )
else:
    df["datetime"] = pandas.to_datetime( df["Date-Time"], format='%Y-%m-%d %H:%M:%S', utc=False ).dt.tz_localize(tz = instrument_tz)
